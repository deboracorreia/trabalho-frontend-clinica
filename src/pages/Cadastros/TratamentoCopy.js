...
const Tratamento = () => {
  ...
  const [tratamento, setTratamento] = useState({ codigo: '', nome: ''});
  ...
  const carregarTratamentos = useCallback(async () => {
    try {
      const resposta = await axios.get(`http://localhost:3001/api/tratamento`, {
        params: { page: paginaAtual, limit: tratamentosPorPagina, search: busca },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editando) {
        await axios.put(`http://localhost:3001/api/tratamento/${editando}`, tratamentos);
        alert('tratamento atualizado com sucesso!');
        setEditando(null);
      } else {
        await axios.post('http://localhost:3001/api/tratamento', tratamentos);
        alert('Tratamento cadastrado com sucesso!');
      }
      setTratamentos({ codigo: '', nome: '' });
      carregarTratamentos();
    } catch (error) {
    ...
    <div className="form-container">
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-group">
          <label>Código:</label>
          <input type="text" name="codigo" value={tratamento.codigo} onChange={handleChange} required />
        </div>
        <div className="form-group">
          <label>Nome:</label>
          <input type="text" name="nome" value={tratamento.nome_tratamento} onChange={handleChange} required />
        </div>
